...
//import com.seed.common.exception.BizException;
 ...
 */
@Slf4j
public final class UtilsFile extends FileUtils {
    /**
     * Buffer的大小
     */
...
//        log.debug("要创建的目录:", paths);
     ...
     * @param newname
     */
    public final static void renameFile(String path, String oldname, String newname) {
        ...
        if (!oldname.equals(newname)) {
            ...
            if (newfile.exists()) {
                throw new RuntimeException(newname + "已经存在！");
            } else {
                oldfile.renameTo(newfile);
            }
        } else {
            throw new RuntimeException("新文件名和旧文件名相同...");
        }
    }
     ...
     * @param newname
     */
    public final static String renameFile(String path, String newname) {
        ...
        if (newfile.exists()) {
            ...
            oldfile.renameTo(newfile);
            return newfile.getAbsoluteFile().toString();
        }
    }

    /**
     * 获取文件的pdf路径 为null时则代表路径不存在
     *
     * @param basePath
     * @param filePath
     * @param fileType pdf html
     * @return
     */
    public final static String getFilePathByType(String basePath, String filePath, String fileType) {
        String attPath = basePath + filePath;
        File file = new File(attPath);
        String pdfName = file.getName().substring(0, file.getName().lastIndexOf(".")) + "." + fileType;
        String pdfPath = file.getParent() + File.separator + fileType + File.separator + pdfName;
        if (!new File(pdfPath).exists()) {
            return null;
        }
        return pdfPath.split(basePath)[1];
    }
}